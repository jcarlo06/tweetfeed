$(function(){
	...
	app.mainController = Backbone.View.extend({
		...
		initialize: function () {
			...
			var j=1;
			while(localStorage.getItem("hist")>=j)
			{
			var query= localStorage.getItem("query"+j);
			var FB=false;
			if(localStorage.getItem("FB"+j)==="true")
				FB=true;
			var Tweet=false;
			if(localStorage.getItem("Twitter"+j)==="true")
				Tweet=true;
			this.searchList.add({
					query:query,
					isFB: FB,
		...
		search: function(query, isFB, isTwitter) {
			...
			var j=localStorage.getItem("hist");
			localStorage.setItem("query"+j, query); 
			localStorage.setItem("FB"+j, isFB); 
			localStorage.setItem("Twitter"+j, isTwitter); 	
				this.searchList.add({
					query: query,
					isFB: isFB,
					isTwitter: isTwitter
